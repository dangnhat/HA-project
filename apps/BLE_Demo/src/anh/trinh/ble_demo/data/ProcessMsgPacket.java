...
//			mContext.mNumOfDev = dataBuf.get();
//			break;
		case NodeInfoDefination.DEV_WITH_INDEX:
			mContext.mDevInfoList = getDeviceList(dataBuf);
			// if receive not enough number of devices
			
			//send message to handler
			Message msg = mContext.mMsgHandler.obtainMessage(NodeInfoDefination.DEV_WITH_INDEX);
...
//    	for(int i = 0; i < 40; i++){
	...
	public ArrayList<BluetoothMessage> createMessageQueue(){
		...
		msgArray[0] = 40;
		msgArray[1] = BTMsgTypeDef.SET;
		msgArray[2] = NodeInfoDefination.NUM_OF_DEVS;
		...
		msgArray[30] = DeviceTypeDef.TEMP_SENSOR; 	// Device ID = TEMP SENSOR
